...
impl SecretProtector for TestProtector {
    fn protect(&self, plaintext: &[u8]) -> Result<Vec<u8>, AppError> {
        Ok(base64::engine::general_purpose::STANDARD.encode(plaintext).into_bytes())
    }
...
pub fn upsert_secret(connection: &Connection, protector: &dyn SecretProtector, key: &str, plaintext: &str) -> Result<(), AppError> {
...
pub fn read_secret(connection: &Connection, protector: &dyn SecretProtector, key: &str) -> Result<Option<String>, AppError> {
    let encrypted: Option<Vec<u8>> = connection
        .query_row("SELECT encrypted_value FROM secure_secrets WHERE secret_key=?1", params![key], |row| row.get(0))
        .optional()?;
    encrypted
        .map(|value| {
            protector
                .unprotect(&value)
                .and_then(|plain| String::from_utf8(plain).map_err(|_| AppError::Database("SECRET_UTF8_INVALID".to_string())))
        })
        .transpose()
}

pub fn delete_secret(connection: &Connection, key: &str) -> Result<(), AppError> {
    connection.execute("DELETE FROM secure_secrets WHERE secret_key=?1", params![key])?;
    Ok(())
}

pub fn store_supabase_session(connection: &Connection, protector: &dyn SecretProtector, session: &SupabaseSession) -> Result<(), AppError> {
    let json = serde_json::to_string(session).map_err(|_| AppError::Database("SUPABASE_SESSION_SERIALIZE_FAILED".to_string()))?;
    upsert_secret(connection, protector, "cloud_supabase_session", &json)
}

pub fn read_supabase_session(connection: &Connection, protector: &dyn SecretProtector) -> Result<Option<SupabaseSession>, AppError> {
    read_secret(connection, protector, "cloud_supabase_session")?
        .map(|json| serde_json::from_str(&json).map_err(|_| AppError::Database("SUPABASE_SESSION_INVALID".to_string())))
        .transpose()
}
...
fn dpapi_protect(plaintext: &[u8]) -> Result<Vec<u8>, AppError> {
    ...
    unsafe {
        ...
        let ok = CryptProtectData(&input, null_mut(), null_mut(), null_mut(), null_mut(), CRYPTPROTECT_UI_FORBIDDEN, &mut output);
        if ok == 0 {
...
fn dpapi_unprotect(ciphertext: &[u8]) -> Result<Vec<u8>, AppError> {
    ...
    unsafe {
        ...
        let mut output = CRYPT_INTEGER_BLOB { cbData: 0, pbData: null_mut() };
        let ok = CryptUnprotectData(&input, null_mut(), null_mut(), null_mut(), null_mut(), CRYPTPROTECT_UI_FORBIDDEN, &mut output);
        if ok == 0 {
